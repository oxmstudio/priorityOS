...
function workspaceColor(workspace) { return workspace === 'personal' ? 'var(--amber)' : 'var(--blue)'; }
function workspaceBg(workspace) { return workspace === 'personal' ? 'var(--amber-d)' : 'var(--blue-d)'; }
function workspaceLabel(workspace) { return workspace === 'personal' ? 'Personal' : 'Business'; }
...
export default function DashboardCalendarPanel({ mode, events = [], setView, localMode = false }) {
  ...
  return <>
    <section className="dash-panel calendar-workspace-panel">
      <div className="dash-section-head">
        <div><h2>Calendar Workspace</h2><p className="panel-sub">One calendar for Business and Personal. Events are labeled by workspace.</p><div className="workspace-legend"><span><i style={{background:workspaceColor("business")}}/>Business</span><span><i style={{background:workspaceColor("personal")}}/>Personal</span></div></div>
        <a className="btn-cal" href={`/dashboard/planner?mode=${mode}${localMode?'&local=1':''}`}>Launch Guided Planner</a>
      </div>
      ...
      <div className="calendar-card-native"><div className="calendar-toolbar-native"><div><button className="btn-ghost" onClick={today}>Today</button><button className="btn-ghost" onClick={()=>shift(-1)}>‹</button><button className="btn-ghost" onClick={()=>shift(1)}>›</button></div><h2>{monthTitle(viewDate)}</h2></div><div className="calendar-head-native">{weekdays.map(day=><div key={day}>{day}</div>)}</div><div className="calendar-grid-native">{days.map(day=>{const key=isoDate(day),dayEvents=byDay[key]||[],currentMonth=day.getMonth()===viewDate.getMonth(),isToday=key===isoDate(new Date()),selected=key===selectedDay;return <button key={key} onClick={()=>setSelectedDay(key)} className={`calendar-cell-native ${selected?'selected':''} ${currentMonth?'':'muted'}`}><span className={isToday?'today-dot':''}>{day.getDate()}</span><div>{dayEvents.slice(0,3).map(event=><em key={`${event.id}-${event.instanceStart}`} style={{background:bgFor(event.quadrant),color:colorFor(event.quadrant)}}><span className="calendar-event-time">{timeLabel(event.instanceStart)}</span><strong className="calendar-event-workspace" style={{background:workspaceBg(event.workspace),color:workspaceColor(event.workspace)}}>{workspaceLabel(event.workspace)}</strong> {event.title}</em>)}{dayEvents.length>3?<small>+{dayEvents.length-3} more</small>:null}</div></button>})}</div></div>
    </section>
    <section className="dash-panel"><div className="dash-section-head"><h2>Selected Day</h2><span className="panel-sub">{new Date(`${selectedDay}T12:00:00`).toLocaleDateString([], {weekday:'long',month:'long',day:'numeric'})}</span></div><ul className="ilist">{selectedEvents.length?selectedEvents.slice().sort((a,b)=>new Date(a.instanceStart)-new Date(b.instanceStart)).map(event=><li key={`${event.id}-${event.instanceStart}`}><span className="idot" style={{background:colorFor(event.quadrant)}}/><span style={{flex:1}}><span style={{display:'block',marginBottom:3}}>{event.title} <strong className="calendar-event-workspace" style={{background:workspaceBg(event.workspace),color:workspaceColor(event.workspace)}}>{workspaceLabel(event.workspace)}</strong></span><span style={{fontSize:11,color:'var(--muted)'}}>{timeLabel(event.instanceStart)} · {qNames[event.quadrant]||'PriorityOS'} · {event.googleEventId?'Google synced':'Internal only'}{event.recRule?' · Recurring':''}</span>{event.notes?<span style={{display:'block',fontSize:11,color:'rgba(255,255,255,.45)',marginTop:4}}>{event.notes}</span>:null}</span></li>):<li className="empty">No events on this day.</li>}</ul></section>

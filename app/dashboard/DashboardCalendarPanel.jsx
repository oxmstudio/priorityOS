      ...
      <div className="calendar-card-native">
        ...
        <div className="calendar-grid-native">{days.map((day) => {
          ...
          return <button key={key} onClick={() => setSelectedDay(key)} className={`calendar-cell-native ${selected ? 'selected' : ''} ${currentMonth ? '' : 'muted'}`}>
            ...
            <div>{dayEvents.slice(0, 3).map((event) => <em key={`${event.id}-${event.instanceStart}`} style={{ background: bgFor(event.quadrant), color: colorFor(event.quadrant) }}><span className="calendar-event-time">{timeLabel(event.instanceStart)}</span><strong className="calendar-event-workspace" style={{background:workspaceBg(event.workspace),color:workspaceColor(event.workspace)}}>{workspaceLabel(event.workspace)}</strong> {event.title}</em>)}{dayEvents.length > 3 ? <small>+{dayEvents.length - 3} more</small> : null}</div>
          </button>;
        })}</div>
      </div>
    </section>

...
function mergeEvents(explicit, tasks) { const seen = new Set(); return [...explicit, ...tasks.map(taskEvent).filter(Boolean)].filter((event) => { const key = event.id || `${event.title}-${event.start}`; if (seen.has(key)) return false; seen.add(key); return true; }); }
...
export default function CalendarClientV2() {
  ...
  async function load(nextMode) {
    ...
    try {
      ...
      const directResult = await fetch(`/api/calendar/internal?mode=${nextMode}&t=${Date.now()}`, { cache: 'no-store' }).then((r) => r.ok ? r.json() : null);
      if (directResult?.state || Array.isArray(directResult?.events)) {
        const workspace = directResult.state || {};
        const explicit = Array.isArray(directResult.events) ? directResult.events : Array.isArray(workspace.calendarEvents) ? workspace.calendarEvents : [];
        const tasks = Array.isArray(workspace.tasks) ? workspace.tasks : [];
        const merged = mergeEvents(explicit, tasks);
        setEvents(merged);
        setReadReceipt({ source: 'internal', explicit: explicit.length, tasks: tasks.length, merged: merged.length });
        return;
      }

      const stateResult = await fetch(`/api/state?mode=${nextMode}&t=${Date.now()}`, { cache: 'no-store' }).then((r) => r.ok ? r.json() : { state: {} });
      const workspace = stateResult.state || {};
      const explicit = Array.isArray(workspace.calendarEvents) ? workspace.calendarEvents : [];
      const tasks = Array.isArray(workspace.tasks) ? workspace.tasks : [];
      const merged = mergeEvents(explicit, tasks);
      setEvents(merged);
      setReadReceipt({ source: 'state', explicit: explicit.length, tasks: tasks.length, merged: merged.length });

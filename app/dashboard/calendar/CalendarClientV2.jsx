...
function colorFor(q) {
  if (q === 'q1') return 'var(--green)';
  if (q === 'q2') return 'var(--blue)';
  if (q === 'q3') return 'var(--amber)';
  if (q === 'q4') return 'var(--red)';
  return 'rgba(255,255,255,.78)';
}
function bgFor(q) {
  if (q === 'q1') return 'var(--green-d)';
  if (q === 'q2') return 'var(--blue-d)';
  if (q === 'q3') return 'var(--amber-d)';
  if (q === 'q4') return 'var(--red-d)';
...
export default function CalendarClientV2() {
  ...
  const byDay = useMemo(() => instances.reduce((acc, event) => {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('priorityos.mode') === 'personal' ? 'personal' : 'business';
    setMode(stored);
    load(stored);
  }, []);
  ...
  async function load(nextMode) {
    ...
    try {
      ...
      const seen = new Set();
      const merged = [...explicit, ...fromTasks].filter((event) => {
  ...
  function switchMode(next) {
    localStorage.setItem('priorityos.mode', next);
    setMode(next);
    load(next);
  }
  ...
  return <>
    <section className="hero" style={{ minHeight: 'auto', paddingBottom: 36 }}>
      ...
      <div className="hero-content">
        ...
        <p className="hero-sub">A month-view account calendar for internal-only blocks and Google-synced commitments.</p>
    ...
    <main className="main" style={{ paddingTop: 20, maxWidth: 1180 }}>
      <section className="lg" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, borderRadius: 15, padding: '14px 16px', marginBottom: 18 }}>
        <div><div className="card-hdr" style={{ marginBottom: 4 }}>Calendar Workspace</div><div style={{ fontSize: 13, color: 'var(--muted)' }}>Internal events are separated by Business and Personal.</div></div>
      ...
      <section className="card" style={{ padding: 0, overflow: 'hidden', borderRadius: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, padding: 16, borderBottom: '.5px solid var(--border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <button className="btn-ghost" onClick={today}>Today</button>
            <button className="btn-ghost" onClick={() => shift(-1)}>‹</button>
            <button className="btn-ghost" onClick={() => shift(1)}>›</button>
            <h2 style={{ fontSize: 24, fontWeight: 500, letterSpacing: '-.7px', marginLeft: 8 }}>{monthTitle(viewDate)}</h2>
          </div>
          <a className="btn-cal" href="/dashboard/planner">+ Create</a>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(0, 1fr))', borderBottom: '.5px solid var(--border)', background: 'rgba(255,255,255,.025)' }}>
          {weekdays.map((day) => <div key={day} style={{ padding: '10px 8px', textAlign: 'center', fontSize: 11, color: 'var(--muted)', fontWeight: 700, borderRight: '.5px solid var(--border)' }}>{day}</div>)}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, minmax(0, 1fr))' }}>
          {days.map((day) => {
            ...
            return <button key={key} onClick={() => setSelectedDay(key)} style={{ minHeight: 126, background: selected ? 'rgba(96,165,250,.08)' : 'rgba(255,255,255,.015)', opacity: currentMonth ? 1 : .38, border: 'none', borderRight: '.5px solid var(--border)', borderBottom: '.5px solid var(--border)', color: 'var(--fg)', padding: 8, textAlign: 'left', cursor: 'pointer', overflow: 'hidden', outline: selected ? '1px solid rgba(96,165,250,.55)' : 'none', fontFamily: 'Inter, sans-serif' }}>
              <span style={{ width: 24, height: 24, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, color: isToday ? '#000' : 'var(--muted)', background: isToday ? 'var(--blue)' : 'transparent', fontWeight: isToday ? 800 : 500 }}>{day.getDate()}</span>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: 7 }}>
                {dayEvents.slice(0, 3).map((event) => <span key={`${event.id}-${event.instanceStart}`} style={{ display: 'block', width: '100%', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', borderRadius: 5, padding: '3px 6px', fontSize: 10, fontWeight: 700, background: bgFor(event.quadrant), color: colorFor(event.quadrant) }}>{timeLabel(event.instanceStart)} {event.title}</span>)}
                {dayEvents.length > 3 ? <span style={{ fontSize: 10, color: 'var(--muted)', paddingLeft: 4 }}>+{dayEvents.length - 3} more</span> : null}
              </div>
            </button>;
          })}
        </div>

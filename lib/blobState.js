...
async function blobResultToJson(result) {
  if (!result) return EMPTY_PRIORITY_STATE;

  // Some versions of @vercel/blob return a Blob-like object directly.
  if (typeof result.text === 'function') {
    return JSON.parse(await result.text());
  }

  // Some versions return { body }, where body may be a Response stream or Blob-like value.
  if (result.body) {
    if (typeof result.body.text === 'function') return JSON.parse(await result.body.text());
    return JSON.parse(await streamToText(result.body));
  }

  // Defensive support for other response shapes.
  if (typeof result.arrayBuffer === 'function') {
    return JSON.parse(Buffer.from(await result.arrayBuffer()).toString('utf8'));
  }

  if (typeof result.content === 'string') return JSON.parse(result.content);
...
export async function readPriorityState(email) {
  try {
    ...
    const state = await blobResultToJson(result);

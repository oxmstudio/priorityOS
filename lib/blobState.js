...
function isBlobMissing(error) {
  ...
  return error?.status === 404 || error?.statusCode === 404 || message.includes('not found') || message.includes('does not exist');
}

async function readBlobJson(pathname) {
  // State must be read fresh after writes so another device can see uploads/layout changes immediately.
  const blob = await get(pathname, { access: 'private', useCache: false });
  if (!blob) return EMPTY_PRIORITY_STATE;
...
export async function readPriorityState(email) {
  ...
  try {
    const state = await readBlobJson(pathname);
    return normalizePriorityState(state);
  } catch (error) {
    console.error('PriorityOS Blob read failed:', {
      pathname,
      message: error?.message,
      status: error?.status,
      statusCode: error?.statusCode
    });
    if (isBlobMissing(error)) return EMPTY_PRIORITY_STATE;
    throw error;
  }
}
...
function normalizeWorkspace(state) {
  ...
  return {
    ...
    dailyNotes: state?.dailyNotes && typeof state.dailyNotes === 'object' && !Array.isArray(state.dailyNotes)
      ? Object.fromEntries(Object.entries(state.dailyNotes).map(([date, notes]) => [date, {
          mindDump: typeof notes?.mindDump === 'string' ? notes.mindDump : '',
          sourceSignals: typeof notes?.sourceSignals === 'string' ? notes.sourceSignals : '',
...
export function normalizePriorityState(state) {
  ...
  const business = normalizeWorkspace(state?.workspaces?.business || (state?.workspaces ? null : state));
  const personal = normalizeWorkspace(state?.workspaces?.personal);
  const legacyCanvas = business.canvas?.items?.length ? business.canvas : personal.canvas?.items?.length ? personal.canvas : null;
  const moodBoard = state?.moodBoard && typeof state.moodBoard === 'object' ? state.moodBoard : legacyCanvas;
  return {
    activeMode,

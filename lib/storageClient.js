...
function ensureLocalServiceWorker() {
  if (typeof window === 'undefined' || !('serviceWorker' in navigator) || !localModeFromLocation()) return;
...
function saveLocalMoodImage(id, file) {
...
function deleteLocalMoodImage(id) {
  return openLocalFileDb().then(db => new Promise((resolve, reject) => {
    const transaction = db.transaction(LOCAL_FILE_STORE, 'readwrite');
...
async function localMoodUpload(request) {
  ...
  for (const file of files) {
    ...
    await saveLocalMoodImage(id, file);
...
export function isLocalMode() {
  ...
  if (local) {
    ensureLocalServiceWorker();
    patchLocalMoodBoardFetch();
  }
  return local;
}
...
function normalizeCanvas(canvas) {
  ...
  if (!Array.isArray(canvas.items)) return canvas;
  return {...canvas, items: canvas.items.map(item => {
    if (!item || typeof item !== 'object' || item.storage !== 'local' || !item.id) return item;
...
function normalizeState(state) {
  ...
  return {activeMode: source.activeMode === 'personal' ? 'personal' : 'business', moodBoard: source.moodBoard && typeof source.moodBoard === 'object' ? source.moodBoard : null, workspaces: {business: workspace(source.workspaces?.business), personal: workspace(source.workspaces?.personal)}};
}
...
export async function hydrateLocalCanvas(canvas) {
  ...
  const items = await Promise.all(normalized.items.map(async item => {
    if (!item?.id || item.storage !== 'local') return item;
...
export async function loadPriorityState(mode = 'business') {
  if (isLocalMode()) {
    const localState = await hydrateLocalState(readLocalState());
    return {state: localState, moodBoard: localState.moodBoard};
  }
...
export async function saveWorkspaceState(mode, state) {
  if (isLocalMode()) {
    ...
    current.workspaces[mode] = normalizeCanvas(state?.canvas) ? {...state, canvas: normalizeCanvas(state.canvas)} : state;
...
export function localAuth() {
  return {loading: false, connected: true, profile: {email: 'local@priorityos.app', name: 'Local PriorityOS'}};
}

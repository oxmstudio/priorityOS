...
const EMPTY_WORKSPACE = {
  values: [], goals: [], tasks: [], calendarEvents: [],
...
export const EMPTY_LOCAL_STATE = {
  activeMode: 'business', moodBoard: null,
  workspaces: {business: structuredClone(EMPTY_WORKSPACE), personal: structuredClone(EMPTY_WORKSPACE)}
};
...
function localModeFromLocation() {
  return typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('local') === '1';
}

function nativeFilesystemAvailable() {
  return typeof window !== 'undefined' && Capacitor.isNativePlatform();
}

function nativeImagePath(id) {
  return `${NATIVE_IMAGE_DIR}/${encodeURIComponent(id)}`;
}
...
function ensureLocalServiceWorker() {
  ...
  if (!serviceWorkerPromise) {
    serviceWorkerPromise = navigator.serviceWorker.register('/priorityos-local-sw.js', {scope: '/'}).catch(error => {
      console.warn('PriorityOS local file service worker registration failed:', error);
      return null;
    });
  }
...
function openLocalFileDb() {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(LOCAL_FILE_STORE)) db.createObjectStore(LOCAL_FILE_STORE);
    };
...
async function deleteNativeMoodImage(id) {
  try { await Filesystem.deleteFile({path: nativeImagePath(id), directory: Directory.Data}); } catch {}
}

async function resolveNativeImageUrl(id) {
  try {
    const result = await Filesystem.getUri({path: nativeImagePath(id), directory: Directory.Data});
...
export async function resolveLocalImageUrl(id) {
  ...
  if (nativeFilesystemAvailable()) {
    const nativeUrl = await resolveNativeImageUrl(id);
    if (nativeUrl) { localObjectUrls.set(id, nativeUrl); return nativeUrl; }
  }
  try {
    ...
    console.warn(`PriorityOS local image read failed for ${id}:`, error);
    return null;
  }
}
...
async function localMoodUpload(request) {
  ...
  for (const file of files) {
    ...
    if (nativeFilesystemAvailable()) {
      ...
      const url = await resolveNativeImageUrl(id);
      ...
      await new Promise((resolve, reject) => {
        const transaction = db.transaction(LOCAL_FILE_STORE, 'readwrite');
        transaction.objectStore(LOCAL_FILE_STORE).put({blob: file, type: file.type, name: file.name, size: file.size}, id);
        transaction.oncomplete = resolve;
        transaction.onerror = () => reject(transaction.error || new Error('Local image could not be saved.'));
      });
...
async function localMoodDelete(request) {
  ...
  if (id) {
    ...
    localObjectUrls.delete(id);
    if (nativeFilesystemAvailable()) await deleteNativeMoodImage(id); else {
      try {
        const db = await openLocalFileDb();
        await new Promise((resolve, reject) => {
          const transaction = db.transaction(LOCAL_FILE_STORE, 'readwrite');
          transaction.objectStore(LOCAL_FILE_STORE).delete(id);
          transaction.oncomplete = resolve;
          transaction.onerror = () => reject(transaction.error || new Error('Local image could not be deleted.'));
        });
      } catch {}
    }
  }
...
export function isLocalMode() {
  const local = localModeFromLocation();
  if (local) { ensureLocalServiceWorker(); patchLocalMoodBoardFetch(); }
  return local;
}
...
function normalizeCanvas(canvas) {
  if (!canvas || typeof canvas !== 'object' || !Array.isArray(canvas.items)) return canvas;
  return {...canvas, items: canvas.items.map(item => {
    if (!item || typeof item !== 'object' || !item.id || !['local','native'].includes(item.storage)) return item;
    return {...item, url: LOCAL_IMAGE_PATH(item.id)};
  })};
}
...
function normalizeState(state) {
  ...
  const workspace = value => ({
    ...clone(EMPTY_WORKSPACE), ...(value && typeof value === 'object' ? value : {}),
...
export async function hydrateLocalCanvas(canvas) {
  ...
  const items = await Promise.all(normalized.items.map(async item => {
    if (!item?.id || !['local','native'].includes(item.storage)) return item;
    const url = await resolveLocalImageUrl(item.id);
    return url ? {...item, url} : item;
  }));
  return {...normalized, items};
}

export async function hydrateLocalState(state) {
  const normalized = normalizeState(state);
  normalized.workspaces.business.canvas = await hydrateLocalCanvas(normalized.workspaces.business.canvas);
  normalized.workspaces.personal.canvas = await hydrateLocalCanvas(normalized.workspaces.personal.canvas);
  if (normalized.moodBoard) normalized.moodBoard = await hydrateLocalCanvas(normalized.moodBoard);
  return normalized;
}

export function readLocalState() {
  if (typeof window === 'undefined') return clone(EMPTY_LOCAL_STATE);
  try { const raw = window.localStorage.getItem(LOCAL_STATE_KEY); return normalizeState(raw ? JSON.parse(raw) : EMPTY_LOCAL_STATE); }
...
export function writeLocalState(state) {
  const normalized = normalizeState(state);
  if (typeof window !== 'undefined') window.localStorage.setItem(LOCAL_STATE_KEY, JSON.stringify(normalized));
  return normalized;
}

export async function loadPriorityState(mode = 'business') {
  if (isLocalMode()) { const localState = await hydrateLocalState(readLocalState()); return {state: localState, moodBoard: localState.moodBoard}; }
  const [workspaceResponse, moodBoardResponse] = await Promise.all([fetch(`/api/state?mode=${mode}&t=${Date.now()}`, {cache: 'no-store'}), fetch(`/api/moodboard/state?t=${Date.now()}`, {cache: 'no-store'})]);
...
export async function saveWorkspaceState(mode, state) {
  if (isLocalMode()) { const current = readLocalState(); current.workspaces[mode] = normalizeCanvas(state?.canvas) ? {...state, canvas: normalizeCanvas(state.canvas)} : state; return writeLocalState(current); }
  const response = await fetch(`/api/state?mode=${mode}`, {method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({mode, state})});
...
export async function saveMoodBoardState(moodBoard) {
  if (isLocalMode()) { const current = readLocalState(); current.moodBoard = moodBoard || null; return writeLocalState(current).moodBoard; }
  const response = await fetch('/api/moodboard/state', {method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({moodBoard: moodBoard || null})});
  if (!response.ok) { const data = await response.json().catch(() => ({})); throw new Error(data.error || 'Mood board save failed.'); }

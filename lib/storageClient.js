...
function patchLocalMoodBoardFetch() {
  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (url.pathname === '/api/moodboard/upload' && request.method.toUpperCase() === 'POST') {
      return localMoodUpload(request);
    }
    if (url.pathname === '/api/moodboard/upload' && request.method.toUpperCase() === 'DELETE') {
      return localMoodDelete(request);
    }
...
async function hydrateLocalCanvas(canvas) {
  ...
  const items = await Promise.all(normalized.items.map(async item => {
    ...
    try {
      const stored = await readLocalMoodImage(item.id);
      if (!stored?.blob) return item;
      let url = localObjectUrls.get(item.id);
      if (!url) {
      ...
      console.warn(`PriorityOS local image restore failed for ${item.id}:`, error);
      return item;
    }
  }));
...
export async function loadPriorityState(mode = 'business') {
  if (isLocalMode()) return {state: await hydrateLocalState(readLocalState()), moodBoard: readLocalState().moodBoard};

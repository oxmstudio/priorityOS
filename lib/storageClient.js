...
async function resolveNativeImageUrl(id) { try { const result = await Filesystem.getUri({path: nativeImagePath(id), directory: Directory.Data}); return Capacitor.convertFileSrc(result.uri); } catch (error) { console.warn(`PriorityOS native image read failed for ${id}:`, error); return null; } }
...
async function localMoodUpload(request) {
  ...
  for (const file of files) {
    ...
    if (nativeFilesystemAvailable()) {
      ...
      const url = URL.createObjectURL(file); localObjectUrls.set(id, url);
...
async function localMoodDelete(request) {
  ...
  if (id) {
    const existing = localObjectUrls.get(id); if (existing?.startsWith('blob:')) URL.revokeObjectURL(existing); localObjectUrls.delete(id);
...
export async function hydrateLocalCanvas(canvas) {
  ...
  const items = await Promise.all(normalized.items.map(async item => { if (!item?.id || !['local','native'].includes(item.storage)) return item; const url = await resolveLocalImageUrl(item.id); return url ? {...item, url, storage: nativeFilesystemAvailable() ? 'native' : item.storage} : item; }));

...
const NATIVE_STATE_PATH = 'priorityos/state.json';
...
async function readNativeState() {
  if (!nativeFilesystemAvailable()) return null;
  try {
    const result = await Filesystem.readFile({path: NATIVE_STATE_PATH, directory: Directory.Data, encoding: Encoding.UTF8});
    if (!result?.data) return null;
    return normalizeState(JSON.parse(result.data));
  } catch (error) {
    return null;
  }
}
...
async function saveNativeState(state) {
  if (!nativeFilesystemAvailable()) return;
  try {
    await Filesystem.writeFile({path: NATIVE_STATE_PATH, data: JSON.stringify(normalizeState(state)), directory: Directory.Data, encoding: Encoding.UTF8, recursive: true});
  } catch (error) {
    console.warn('PriorityOS native state save failed:', error);
  }
}
...
export async function hydrateLocalState(state) {
  ...
  if (nativeFilesystemAvailable()) {
    ...
    if (nativeState) {
      normalized = nativeState;
    } else {
      await saveNativeState(normalized);
    }
...
export function writeLocalState(state) { const normalized = normalizeState(state); if (typeof window !== 'undefined') window.localStorage.setItem(LOCAL_STATE_KEY, JSON.stringify(normalized)); if (nativeFilesystemAvailable()) void saveNativeState(normalized); return normalized; }
